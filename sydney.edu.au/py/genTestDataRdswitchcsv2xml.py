#!/usr/bin/python
# Author Keir Vaughan-Taylor     Mon Dec  5 07:37:49 AEDT 2016
# Based on rdswitchcsv2xml.py however
# Generates limited number of XML data based on  10 lsne of data for testing
# 
import sys,csv,os
import codecs
import pdb
...
def rifListOut(rifdef,idt):
   ...
   lastrow=[]
   linectr=0
   for row in datreader:           # Main loop to read CSV data
      linectr+=1
      if linectr > numTestlines: break         # This is for outputing a limited set of test data
...
def groupOut(rifdef,idt):
   if type(rifdef) is dict:
      idt+=1
      groupItem=next(iter(rifdef))         # Assumes a dictionary item containing a list
      groupTag="<%s>\n" % groupItem
      emit(groupTag,idt)
...
rifout=xmlDestDir+"/testData_"+idKeyWord+".xml"
...
with codecs.open(rifout,"w",encoding='utf-8',errors='replace') as rifoutfd:
   with open(rawDatIn,"rU") as fieldDatafd:
      ...
      headers=datreader.next()                 #Assumes first line of CSV data is a header, headers used to be XML tages in output
      headlen=len(headers)
